...
function sortByDate(nodes) {
  ...
  duplicate.sort((nodeA, nodeB) => {
    ...
    const hasADate = typeof publishDateA !== 'undefined'
    const hasBDate = typeof publishDateB !== 'undefined'
    if (!hasADate && !hasBDate) {
      ...
      const aDate = parseDate(publishDateA)
      const bDate = parseDate(publishDateB)
      if (aDate < bDate) {
        return 1
      } if (bDate < aDate) {
        return -1
